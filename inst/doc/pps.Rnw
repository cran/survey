...
\subsection*{H\'ajek-type estimators}
...
The Yates--Grundy estimator appears to be preferred.

In practice, $\pi_{ij}$ may not be available and various approximations have been proposed.  The (truncated) Hartley--Rao approximation is
...
that requires only the sample $\pi_i$.

Overton's approximation (REF) is
